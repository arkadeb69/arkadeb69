# Hi, I'm Arkadeb Thokdar 

### Web Developer • Mechanical Engineering Student • MLOps & API Security Enthusiast

📍 West Bengal, India

[![Gmail](https://img.shields.io/badge/Gmail-arkayt30%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:arkayt30@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Arkadeb%20Thokdar-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/arkadebthokdar)
![](https://komarev.com/ghpvc/?username=arkadebgit&label=Visitors&color=6A5ACD&style=for-the-badge)

---

##  About Me

I enjoy building modern web applications with a strong focus on **AI integration**, **serverless architecture**, **API security**, and **offline-first experiences**.

Currently pursuing **B.Tech in Mechanical Engineering** at **Cooch Behar Government Engineering College** (Expected Graduation: **2029**).

---

#  Tech Stack

### Languages

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-A8B9CC?style=for-the-badge&logo=c&logoColor=black)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS-1572B6?style=for-the-badge&logo=css3&logoColor=white)

### Frontend

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white)

### Backend

![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Express](https://img.shields.io/badge/Express-000000?style=for-the-badge&logo=express&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)

### Cloud & Database

![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black)
![Firestore](https://img.shields.io/badge/Firestore-FFCA28?style=for-the-badge&logo=firebase&logoColor=black)
![Cloud Storage](https://img.shields.io/badge/Cloud%20Storage-4285F4?style=for-the-badge&logo=googlecloud&logoColor=white)
![Vercel](https://img.shields.io/badge/Vercel-000000?style=for-the-badge&logo=vercel&logoColor=white)

### AI & APIs

![Google Gemini](https://img.shields.io/badge/Google%20Gemini-8E75B2?style=for-the-badge&logo=google-gemini&logoColor=white)
![REST API](https://img.shields.io/badge/REST_API-02569B?style=for-the-badge)
![Resend](https://img.shields.io/badge/Resend-000000?style=for-the-badge&logo=resend&logoColor=white)
![MLOps](https://img.shields.io/badge/MLOps-FF6F00?style=for-the-badge)

### Developer Tools

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)
![Cloudflare](https://img.shields.io/badge/Cloudflare-F38020?style=for-the-badge&logo=cloudflare&logoColor=white)
![Apigee](https://img.shields.io/badge/Apigee-FF6D00?style=for-the-badge&logo=googlecloud&logoColor=white)

---

#  Featured Projects

# Featured Projects

## 📄 [Resume X AI](https://github.com/arkadeb69/resume-x-ai)

AI-powered resume analysis platform that compares resumes with job roles, identifies skill gaps, and generates personalized learning roadmaps.

**Tech Used**

- Node.js
- Express.js
- Google Gemini API
- Resend API
- pdf.js
- Tesseract.js

**Highlights**

- Client-side OCR processing
- Structured JSON output using Gemini
- Zero-server document parsing

---

## 📸 [CGEC Capture Crew Portal](https://github.com/arkadeb69/CGEC_Capture_Crew)

Official photography club portal featuring recruitment, gallery management, certificate verification, and event showcases.

**Tech Used**

- React
- Vite
- Firebase
- Resend API
- Vercel Serverless

**Highlights**

- Dynamic SEO meta generation
- Edge Functions
- Firestore REST integration

---

## 💳 [Credit Bill Manage](https://github.com/arkadeb69/Credit-Bill-manage)

Offline-first personal finance dashboard for managing credit cards and bill tracking.

**Tech Used**

- React 19
- Vite
- HTML5
- CSS3
- Browser LocalStorage

**Highlights**

- Offline-first architecture
- Automatic bill status engine
- Smart payment aging calculation

---

## 🔒 [PC Remote Lock](https://github.com/arkadeb69/win-remote-toolkit)

Remotely lock, sleep, or shutdown a Windows PC securely from any device.

**Tech Used**

- Python
- Flask
- Windows API
- Cloudflare Tunnel

**Highlights**

- Native Windows API integration
- Secure password hashing
- Background daemon with Cloudflare Tunnel
---

#  Certifications

- MLOps for Generative AI
- API Security using Apigee API Platform

---

#  Additional Experience

Besides software development, I've also worked as a freelance creative professional.

- Video Editing
- Graphic Design
- YouTube Branding
- Social Media Content
- Short-form Content Production

Worked with brands and creators including **Pixel&People**, **Wrongpencilad**, and **Abir Debnath**, delivering branding assets and short-form content with an emphasis on quality and timely delivery.
